...
export const TRANSACTION_SYNC_PROPERTIES = [
  "categoryId",
  "amountCents",
  "title",
  "note",
...
export type TransactionSyncProperty =
  (typeof TRANSACTION_SYNC_PROPERTIES)[number];
...
export type SyncTarget =
  ...
  | { kind: "transaction"; id: string; property?: TransactionSyncProperty };
...
export function parseSyncTarget(value: string): SyncTarget | null {
  ...
  const match = value.match(
    /^(benefit|expense|transaction):([^:]+)(?::([A-Za-z]+))?$/,
  );
  if (!match) return null;
  const kind = match[1];
  if (kind !== "benefit" && kind !== "expense" && kind !== "transaction")
    return null;
  ...
  if (kind === "benefit")
    return { kind, id, property: match[3] as BenefitSyncProperty };
  if (kind === "expense")
    return { kind, id, property: match[3] as ExpenseSyncProperty };
  return { kind, id, property: match[3] as TransactionSyncProperty };
}
